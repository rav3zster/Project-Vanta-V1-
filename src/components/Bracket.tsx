...
function MatchCard({ match, find }: { match: Match; find: (id: string | null) => Team | null }) {
  ...
  return (
    <div
      className={`group relative w-full sm:w-64 border bg-surface transition-colors hover:border-border-strong ${
  ...
  return (
    <div className={`flex flex-col ${className}`}>
      ...
      <div className="flex flex-1 flex-col justify-around gap-6">
        {ms.map((m, i) => (
          <Reveal key={m.id} delay={baseDelay + i * 90}>
            <MatchCard match={m} find={find} />
          </Reveal>
        ))}
      </div>
    </div>
