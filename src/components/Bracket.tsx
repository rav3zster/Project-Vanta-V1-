...
function MatchCard({ match, find }: { match: Match; find: (id: string | null) => Team | null }) {
  ...
  return (
    <div
      className={`w-64 border bg-surface transition-colors hover:border-border-strong ${
...
export function Bracket({ teams, matches }: { teams?: Team[]; matches?: Match[] }) {
  ...
  return (
    <div className="grain relative overflow-x-auto border border-border bg-surface/40 p-6 sm:p-8">
      <div className="relative z-10 flex min-w-[860px] gap-10">
        <Column title="QUARTERFINALS" ms={qf} find={find} className="w-64" />
        <Column title="SEMIFINALS" ms={sf} find={find} className="w-64" />
        <div className="flex flex-col">
          <Column title="FINAL" ms={final} find={find} className="w-64 flex-1" />
          <div className="mt-6 w-64 border border-accent/30 bg-accent/5 px-3 py-3">
